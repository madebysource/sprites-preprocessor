...
var sprite = function(options) {
  ...
  var urlRegex = new RegExp("url\\((?:'|\")?(" + escapeRegExp(options.prefix) + ".*?)(?:'|\")?\\)", 'gi');
  ...
  stream._write = function(file, enc, callback) {
    ...
    spritesmith({ src: paths.map(createFullPath) }, function(err, result) {
      ...
      paths.forEach(function(item) {
        ...
        var regex = new RegExp("url\\((?:'|\")?" + escapeRegExp(options.prefix) + escapeRegExp(item) + ".*?(?:'|\")?\\)(?:(.*?|\\n*?|\\r*?))(;|})");
        content = content.replace(regex, 'url(' + options.name + ')$1;\n' + createCSSPropertiesFor(coords) + '$2');
      });
